...
#np.delete(a, 0)
a = np.genfromtxt('simulation/sim_std_err.csv', delimiter=',')
...
np.histogram(a,bins = bin_arr) 
hist,bins = np.histogram(a_all,bins = bin_arr) 

pl.hist(a,bins = bin_arr, edgecolor='black', linewidth=1.2) 
#pl.plot(ln, bin_arr)
pl.title('Errors added to range measurements in simulation')

pl.savefig('simulated_err_hist.png')
